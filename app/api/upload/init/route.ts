...
export async function POST(request: NextRequest) {
  ...
  const userName = cookieStore.get("choir-user-name")?.value;
  ...
  try {
    console.log("[upload/init] Starting upload:", { fileName, mimeType, fileSize, userName });
    const sessionUri = await initResumableUpload(fileName, mimeType, fileSize, userName);
